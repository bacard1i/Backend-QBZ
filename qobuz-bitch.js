...
// ==================== TIDAL CREDENTIALS ====================
var TIDAL_CLIENT_ID = "xhUoz79ynuzFFdQb";
var TIDAL_CLIENT_SECRET = "EUYCL1RNrBi0vNDfphr6o4OUYhpibtgD1rr28n9wsaA=";
var TIDAL_TOKEN = "eyJraWQiOiJ2OU1GbFhqWSIsImFsZyI6IkVTMjU2In0..qBOKoDn_ocAPYZ7up-Rp5kvDk7UtLO9daFsO0314t3OlqVDof-Ua3the9heQ7TEfKFQR3QhU5UdP5UVLGGpd7A";
var TIDAL_COUNTRY = "CA";
...
// ==================== SEARCH (Qobuz + Tidal direct + ISRC dedup) ====================
var searchTracks = function(query, limit) {
  if (!limit) limit = 25;

  var qobuzPromise = qobuz("/track/search", { query: query, limit: limit * 2 }).then(data => {
    var items = (data.tracks && data.tracks.items) || [];
    return items.slice(0, limit * 2).map(t => ({
      id: String(t.id),
      title: t.title,
  ...
  var tidalPromise = fetch("https://api.tidal.com/v1/search/tracks?query=" + encodeURIComponent(query) +
                           "&limit=" + (limit * 2) + "&countryCode=" + TIDAL_COUNTRY, {
    headers: { "Authorization": "Bearer " + TIDAL_TOKEN }
  })
  .then(r => r.ok ? r.json() : { items: [] })
  .then(data => (data.items || []).map(t => ({
    id: String(t.id),
    title: t.title,
  ...
  return Promise.all([qobuzPromise, tidalPromise]).then(([qobuzTracks, tidalTracks]) => {
    var map = new Map();
    ...
    var merged = Array.from(map.values()).slice(0, limit);
    return { tracks: merged, total: merged.length };
  });
};

// ==================== STREAM (Qobuz first → Tidal fallback) ====================
var getTrackStreamUrl = async function(trackId, retry = 0) {
    ...
    if (r.ok) {
      ...
      if (data.url) {
        var result = {
          streamUrl: data.url,
          track: {
            audioQuality: (data.bit_depth || 24) + "-bit / " + (data.sample_rate || 0) + " kHz",
            source: "Qobuz"
          }
        };
  ...
  try {
    return {
      ...
      note: "Tidal direct streaming not fully supported yet"
    };
  } catch (e) {}
...
return {
  ...
  author: "bacardii",
  version: "2.0",
  description: "Qobuz Hi-Res + Direct Tidal Fallback (Merged Search)",
  labels: ["QOBUZ", "TIDAL", "HI-RES", "MERGED", "PRELOAD"],
