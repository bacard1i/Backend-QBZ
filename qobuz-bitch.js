...
// Tidal fallback via your worker
var TIDAL_WORKER = "https://pushit.hatestar.workers.dev";
...
// ==================== SEARCH (Qobuz + Tidal with ISRC deduplication) ====================
var searchTracks = function(query, limit) {
  ...
  var tidalPromise = fetch(TIDAL_WORKER + "/search?q=" + encodeURIComponent(query) + "&limit=" + (limit * 2))
    .then(r => r.json())
    .then(data => (data.tracks || []).map(t => ({
      id: String(t.id),
      title: t.title,
  ...
  return Promise.all([qobuzPromise, tidalPromise]).then(([qobuzTracks, tidalTracks]) => {
    ...
    // Tidal only if not already present (ISRC dedup)
...
var getTrackStreamUrl = async function(trackId, retry = 0) {
  ...
  // Tidal fallback
  try {
    var tidalRes = await fetch(TIDAL_WORKER + "/stream/" + trackId);
    var tidalData = await tidalRes.json();

    if (tidalData.streamUrl) {
      var result = {
        streamUrl: tidalData.streamUrl,
        track: {
          audioQuality: tidalData.track?.audioQuality || "HiFi",
          source: "Tidal"
        }
      };
      streamCache[cacheKey] = result;
...
// ==================== PRELOAD + ALBUM (kept from your working version) ====================
...
var preloadTrack = function(trackId) {
  ...
  if (!preloadQueue.includes(trackId)) preloadQueue.unshift(trackId);
...
return {
  ...
  author: "bacardii",
  version: "1.5",
  description: "Qobuz Hi-Res Primary + Tidal Fallback (ISRC merged search + playback)",
  labels: ["QOBUZ", "TIDAL", "HI-RES", "PRELOAD", "INSTANT", "MERGED"],
